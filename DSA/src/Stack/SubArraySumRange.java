...
public class SubArraySumRange {
	...
	public static void main(String[] args) {
		...
		System.out.println("the sub array sum range is: "+subArrayRanges(nums));
	}
	 public static long subArrayRanges(int[] nums) {
	        long max = sumMax(nums);
	        long min = sumMin(nums);
	        return max - min;
	    }
	    private static long sumMax(int arr[]){
	        int pgei[] = findPGEI(arr);
	        int ngei[] = findNGEI(arr);
	        long total = 0;
	        for(int i=0;i<arr.length;i++){
	            int left = i - pgei[i];
	            int right = ngei[i] - i;
	            total = (total + (long)left * (long)right * (long)arr[i]);
	        }return total;
	    }
	    ...
	    private static int[] findNSEI(int arr[]){
	        int n = arr.length;
	        int nsei[] = new int[n];
	        Stack<Integer> st = new Stack<>();
	        for(int i=n-1;i>=0;i--){
	    ...
	    private static int[] findPGEI(int arr[]){
	        int n = arr.length;
	        int pgei[] = new int[n];
	        Stack<Integer> st = new Stack<>();
	        for(int i =0;i<n;i++){
	            while(!st.isEmpty() && arr[st.peek()] < arr[i])
	                st.pop();
	            pgei[i] = st.isEmpty() ? -1:st.peek();
	            st.push(i);
	        }return pgei;
	    }

	    private static int[] findNGEI(int arr[]){
	        int n = arr.length;
	        int ngei[] = new int[n];
	        Stack<Integer> st = new Stack<>();
	        for(int i= n-1;i>=0;i--){
